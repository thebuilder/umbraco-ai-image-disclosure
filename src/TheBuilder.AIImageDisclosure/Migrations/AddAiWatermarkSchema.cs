...
internal sealed class AddAiWatermarkSchema(
    ...
    IShortStringHelper shortStringHelper) : AsyncMigrationBase(context)
{
    protected override async Task MigrateAsync()
    {
        ...
        var existing = imageMediaType.PropertyTypes.FirstOrDefault(property =>
            property.Alias == Constants.AiWatermarkPropertyAlias);
        if (existing is not null)
        {
            AiImageDisclosureSchemaGuard.EnsurePropertyUsesDataType(
                existing, dataType.Key, Constants.AiWatermarkPropertyAlias);
            return;
        }
        ...
        var property = new PropertyType(shortStringHelper, dataType, Constants.AiWatermarkPropertyAlias)
        {
            Name = "AI watermark",
            Description = Constants.AiWatermarkPropertyDescription,
            Mandatory = false,
        ...
        if (!result.Success)
            throw new InvalidOperationException($"Could not add the AI watermark property: {result.Result}.");
    }
}
